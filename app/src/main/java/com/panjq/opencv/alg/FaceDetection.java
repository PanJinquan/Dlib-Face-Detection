...
public class FaceDetection {
    ...
    public  native boolean initFaceDetection(String mDlibModel);
    public native int faceDetection(long matAddrSrcImage, long matAddrDestImage);
     ...
     *
     */
    public Bitmap faceDetection(Bitmap origImage) {
        Log.i(TAG, "called JNI:faceDetection ");
        int w=origImage.getWidth();
        int h=origImage.getHeight();
        Mat origMat = new Mat();
        Mat destMat = new Mat();
        Utils.bitmapToMat(origImage, origMat);
        int face_num=faceDetection(origMat.getNativeObjAddr(), destMat.getNativeObjAddr());
        ...
        Log.i(TAG, "faceDetection called successfully,face num="+face_num);
        return bitImage;
    }
